...
#define port_cs_mmc	BIT(21)		// port 0.1

#define GPIO_IOSET	FIO1SET
#define GPIO_IOCLR	FIO1CLR
#define GPIO_IODIR	FIO1DIR
	...
	SSP0CR1 = 0x00;
   	...
   	if (min == 1)
   	{
  		...
  		//PCONP |= (1 << 21);
   		...
   		SSP0CR0 = 0x6000 | 0x07; // | BIT(6) | BIT(7);
   	}
   	else
   		//SSP1CR0 = 0x400 | 0x07;		// SCR = 0x400 dapat 5 Mhz, cocok karena 60 Mhz / (2 * (4+1))
   		SSP0CR0 = 0x200 | 0x07;// | BIT(6) | BIT(7);		// SCR = 0x200 dapat 10 Mhz, cocok karena 60 Mhz / (2 * (2+1))
...
void kirim_word_mmc(unsigned char t)
{
	while ( (SSP0SR & (SSP1TNF|SSP1BSY)) != SSP1TNF );
	SSP0DR = t;
	
	while ( (SSP0SR & (SSP1BSY|SSP1RNE)) != SSP1RNE );
